...
public class DataApiResponse<T> extends ApiResponse<T> {
  ...
  /**
   * Creates a successful {@link DataApiResponse} instance with the provided data and message.
   *
  ...
  /**
   * Creates a {@link DataApiResponse} representing a bad request (HTTP 400) response.
   *
   * @param data the {@link BadRequestResponse} payload containing details about the bad request
   * @param message a descriptive message explaining the reason for the bad request
   * @return a {@link DataApiResponse} with success set to false, status code 400, the provided
   *     message, and data
   */
